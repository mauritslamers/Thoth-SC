...
ThothSC.ModelGraphBuilder = SC.Object.extend({
  ...
  relationsToSend: function(){
    var filter = function(rel){
      if(rel.preventInclusion) return false;
      if(rel.shouldSend){
  ...
  relationsFor: function(record){
    ...
    parser = function(rel){
      if(rel){
        propName = rel.propertyKey || rel.propertyName;
        if(propName && record && rel.isMaster){ // don't send out slave data
          propValue = record[propName];
          propValue = rel.isChildRecord? record[propName][rel.primaryKey]: propValue; // take the id of the childrecord
  ...
  _handleToMany: function(prop,propName){
    ...
    if(prop.kindOf(SC.ManyAttribute) || prop.kindOf(SC.ChildrenAttribute)){
      ThothSC.modelCache.store(prop.typeClass()); // make sure all related models are also known
      ...
      if(oppositeRecType === String.prototype){
        ...
        if(prop.oppositeProperty){
          ...
          if(!prop.isMaster){ 
            ...
            if(oppositeProp && oppositeProp.key) ret.relationKey = oppositeProp.key;
          }
          else {
            if(prop.isUpdatable === true) ret.isUpdatable = true;
          }
  ...
  _handleToOne: function(prop,propName){
    ...
    if(prop.kindOf(SC.SingleAttribute) || prop.kindOf(SC.ChildAttribute)){
      ...
      if(oppositePropName && oppositeRecType[oppositePropName]){
        ...
        if(prop.forceInclusion) ret.shouldSend = true;
        this._relations.push(ret); 
        return true;
      }
      else {
        SC.Logger.log("ThothSC: You defined a relation, but the opposite side could not be found.");
        SC.Logger.log("Relation: " + propName + ' to ' + oppositePropName);
        return true;
      }
    }
  },
  ...
  contentDidChange: function(){
    ...
    if(!this._resource){
      SC.Logger.log('Trying to create a model graph without a resource on the model! Is this model actually defined?');
      return;
    } 
