...
ThothSC.SocketIOClient = ThothSC.BaseClient.extend({
  ...
  connect: function(){  
    ...
    var me = this;  
    var opts = {};
    if(!this.dataSource.useWebSocket){
      opts.transports = ['xhr-polling', 'htmlfile', 'jsonp-polling'];
    }
    // var opts = {
    //   'connect timeout': 1000
    // };  
    var prefix = this.get('ThothURLPrefix');
    var protocol = this.dataSource.get('useSSL')? "https://": "http://";
    var url = protocol + this.get('host');
    if(!this.isConnected){
